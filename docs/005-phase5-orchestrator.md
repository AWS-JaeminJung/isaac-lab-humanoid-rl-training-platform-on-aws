...
| GPU 스케일 패턴 | Baseline ASG 상시 2대 + Karpenter Burst 0→N | Baseline 16 GPU 즉시 가용. Burst는 Spot 우선, OD fallback으로 탄력적 확장 |
...
│  2. Check gpu_quota (researcher: 16, engineer: 32)        │
...
│ (Mgmt Node)  │      │                                     │
│              │      │  Baseline (ASG): g7e.48xlarge x2    │
│ Dashboard    │◄────▶│  Burst (Karpenter): Spot/OD 0~N     │
│ GCS          │ :6379│                                     │
│ :8265        │      │  Worker 1: 8x L40S (Baseline)       │
│              │      │  Worker 2: 8x L40S (Baseline/Burst) │
│              │      │  ...                                │
└──────────────┘      └─────────────────────────────────────┘
```
...
RayJob created (gpu: 8, num_nodes: 2)
  │
  ▼
OSMO Controller: 워크로드 분류
  │  멀티노드/장시간 → nodeSelector: gpu-tier=baseline
  │  HPO/단기 실험 → toleration만 (Karpenter burst)
  ▼
Kubernetes Scheduler
  │  Pending Pod (nvidia.com/gpu: 8)
  │
  ├─ [Baseline 노드 여유 있음] → 즉시 스케줄링 (수 초)
  │
  └─ [여유 없음] → Karpenter Burst
       │  1. Detect Pending Pod
       │  2. gpu-burst-spot (weight=10) 먼저 시도
       │  3. Spot 없으면 → gpu-burst-od (On-Demand fallback)
       │  4. EC2 API → launch g7e.48xlarge (3~7분)
       ▼
GPU Node Ready → Pod Scheduled → training starts
  │
  │  (training complete)
  ▼
Ray Cluster deleted
  │
  ├─ [Baseline 노드] → 유지 (ASG min=2, 항상 가동)
  └─ [Burst 노드] → Karpenter consolidation (10m) → 노드 종료
```
...
data:
  gpu-quota-policy.yaml: |
    ...
    gpu_quota:
      ...
      role_quotas:
        # admin: Baseline 16 GPU + Burst 전체 (최대 80 GPU)
        admin:
          max_gpus: 80
          max_concurrent_jobs: 20
          priority: 100
          description: "관리자 — 전체 GPU 클러스터 접근 (Baseline + Burst)"

        # engineer: Baseline 16 GPU + Burst 추가 (최대 32 GPU = 4 x g7e.48xlarge)
        engineer:
          max_gpus: 32
        ...
        # researcher: Baseline에서 최대 16 GPU (2 x g7e.48xlarge)
        researcher:
          max_gpus: 16
