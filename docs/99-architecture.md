...
Virtual Private Gateway (vgw)
  │
  ├── GPU Subnet (10.100.0.0/24)         ← g6e.48xlarge x10, EFA
  ...
  └── Reserved Subnet (10.100.3.0/24)    ← 확장용
...
EKS Control Plane (Private Endpoint Only)
  ...
  └── GPU Nodes (Karpenter provisioned)
        └── g6e.48xlarge x0~10 (8x L40S + EFA)
...
KubeRay Operator → Ray Cluster (Head + Workers)
  │
  ▼
Karpenter → g6e.48xlarge 프로비저닝 (8x L40S, EFA, FSx mount)
```
...
User Notebook (CPU only, 2C/4Gi, max 10 concurrent)
  │
  ├──→ OSMO API (워크플로우 제출)
  ├──→ ClickHouse (실시간 SQL 모니터링)
  ├──→ MLflow (실험 관리)
  └──→ S3 (체크포인트 접근 via boto3)
```
...
4. KubeRay → Karpenter → g6e.48xlarge 프로비저닝
