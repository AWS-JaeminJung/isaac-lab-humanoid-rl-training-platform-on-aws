...
| GPU 컴퓨팅 | [EKS Managed Node Group](https://docs.aws.amazon.com/eks/latest/userguide/managed-node-groups.html) + [Karpenter](https://karpenter.sh/docs/) | Baseline: g7e.48xlarge On-Demand 2대 (ASG 상시), Burst: Karpenter Spot+OD 0~N대 |
...
AWS VPC (10.100.0.0/21, Single AZ)
  ...
  │   ├── Baseline: g7e.48xlarge x2 On-Demand (ASG, 항상 16 GPU)
  │   └── Burst: g7e.48xlarge x0~N Karpenter (Spot 우선, OD fallback)
...
| Karpenter | 1 | 1 | 1Gi | GPU Burst 오토스케일 (Spot/OD) |
...
| GPU 오토스케일 범위 | Baseline 2대(ASG) + Karpenter Burst(Spot/OD) | **결정됨** |
| Spot 인스턴스 허용 여부 | Burst에만 Spot 허용 (HPO, 단기 실험), 장시간/멀티노드는 Baseline OD | **결정됨** |
| 비용 최적화 | OD → 3개월 후 Compute SP 1yr (g7e 전환 유연성) | **결정됨** |
...
| 3 | **Bridge** | [003-phase3-bridge.md](003-phase3-bridge.md) | EKS Hybrid Nodes — On-Prem GPU 등록, Hybrid Nodes Gateway, DX 연결 확인 |
