...
| 컴포넌트 | 도구 | 역할 |
|----------|------|------|
| 학습 프레임워크 | Isaac Lab + rsl_rl | H1 humanoid PPO 학습 |
| 분산학습 / HPO | Ray (TorchTrainer, Tune) | 멀티노드 학습, 하이퍼파라미터 최적화 |
| 워크플로우 오케스트레이션 | NVIDIA OSMO + KubeRay | 학습 작업 제출/관리 |
| 컨테이너 오케스트레이션 | Amazon EKS | GPU/CPU 노드 관리 |
| GPU 오토스케일 | Karpenter | g6e.48xlarge 0-10대 |
| 고속 스토리지 | FSx for Lustre | 체크포인트, 멀티노드 공유 |
| 실험 추적 / 모델 레지스트리 | MLflow + S3 | 실험 메타데이터, 모델 버전 관리, 아티팩트 |
| 학습 로그 수집/분석 | ClickHouse + Fluent Bit | iteration 메트릭, raw 로그, SQL 분석 |
| 모니터링 | Prometheus + Grafana + DCGM Exporter | GPU util, 인프라 메트릭, 대시보드, 알림 |
| 연구자 인터페이스 | JupyterHub | 학습 제출, 결과 분석, 시각화 |
| 인증 | Keycloak (AD 연동) | OIDC, 역할 기반 접근 제어 |
| 네트워크 | Direct Connect + Internal ALB | Private 전용, On-Prem 연결 |
...
| Phase | 이름 | 문서 | 내용 |
|:-----:|------|------|------|
| 1 | **Foundation** | [phase1-foundation.md](phase1-foundation.md) | 네트워크 기반 — VPC, 서브넷, DX, VPN, VPC Endpoints, SG, Route53, TLS |
