...
│  (Browser) ──────────────┼── DX + ALB ────►│  │                                          │
│                          │                 │  ├── GPU Baseline (ASG, 2x g7e.48xlarge)    │
│                          │                 │  │   └── 16 GPU always-on, On-Demand        │
│                          │                 │  └── GPU Burst (Karpenter, 0→N)             │
│                          │                 │      └── Spot preferred + OD fallback        │
│                          │                 │                                             │
│                          │                 │  Storage                                    │
...
| Roles | researcher (gpu_quota=16), engineer (gpu_quota=32), admin (gpu_quota=80), viewer (gpu_quota=0) |
| OIDC Clients | jupyterhub, grafana, mlflow, ray-dashboard, osmo-api |
| Networking | ALB Ingress + Route53: keycloak.isaac-lab.internal |
...
```
User (browser) → keycloak.isaac-lab.internal → AD LDAP lookup → JWT issued
...
| Client | Redirect URI |
|--------|-------------|
| jupyterhub | `https://jupyter.isaac-lab.internal/hub/oauth_callback` |
| grafana | `https://grafana.isaac-lab.internal/login/generic_oauth` |
| mlflow | `https://mlflow.isaac-lab.internal/callback` |
| ray-dashboard | `https://ray.isaac-lab.internal/oauth/callback` |
...
| Networking | ALB + Route53: osmo.isaac-lab.internal, ray.isaac-lab.internal |
...
| Networking | ALB + Route53: mlflow.isaac-lab.internal |
...
**Access**: Users authenticate via `mlflow.isaac-lab.internal` → Keycloak OIDC login → MLflow UI. The OAuth2 Proxy sits in front as the ALB target.
...
| Networking | ALB + Route53: grafana.isaac-lab.internal |
...
| Networking | ALB + Route53: jupyter.isaac-lab.internal |
...
| `domain` | `isaac-lab.internal` | Private hosted zone domain |
| `s3_prefix` | `isaac-lab-prod` | S3 bucket naming prefix |
| `environment` | `prod` | Environment identifier (prod, staging, dev) |
...
The cluster has three distinct types of GPU nodes. Which node a training job runs on is determined by `nodeSelector` and `tolerations` in the RayJob manifest.
...
│
├── GPU Baseline Nodes (ASG, Phase 02)
│   ├── label:      node-type=gpu
│   ├── taint:      nvidia.com/gpu:NoSchedule
│   ├── instance:   g7e.48xlarge x2 (8x L40S each, 16 GPU total)
│   └── always-on:  min=max=2, On-Demand, immediate scheduling
│
└── GPU Burst Nodes (Karpenter, Phase 02)
    ├── label:      node-type=gpu
    ├── taint:      nvidia.com/gpu:NoSchedule
    ├── gpu-burst-spot (weight=10): g7e/g6e Spot, max 80 GPU
    ├── gpu-burst-od   (weight=1):  g7e.48xlarge OD fallback, max 40 GPU
...
| Multi-GPU (8 GPU) | Baseline/Burst | `node-type: gpu` | `nvidia.com/gpu` |
| Multi-Node (16 GPU) | Baseline+Burst | `node-type: gpu` | `nvidia.com/gpu` |
| HPO (variable) | Baseline+Burst | `node-type: gpu` | `nvidia.com/gpu` |
...
> **Common mistake**: Using `node-type: gpu` for single-GPU jobs will schedule on a Baseline node or trigger Karpenter to provision a full g7e.48xlarge (8 GPUs) even though only 1 GPU is needed. For quick experiments and iteration, always target On-Prem nodes with `node-type: onprem-gpu`.

**GPU scheduling flow**:

```
Training job submitted → RayJob created → Pods pending (GPU request)
  → Baseline ASG (2x g7e.48xlarge, 16 GPU always-on): immediate scheduling if capacity available
  → If Baseline full → Karpenter detects pending → Launches Burst node (Spot/OD)
  → Training runs → Job completes → TTL cleanup → Burst node terminated (consolidateAfter: 5m)
  → Baseline nodes remain always-on
```

### Authentication Flow

```
User → Browser → keycloak.isaac-lab.internal
...
|------|-----------|----------|
| researcher | 16 | IsaacLab-Researchers |
| engineer | 32 | IsaacLab-Engineers |
| admin | 80 | IsaacLab-Admins |
