...
# ---------------------------------------------------------------------------
# GPU Baseline Node Group (ASG, always-on)
# ---------------------------------------------------------------------------

variable "gpu_baseline_instance_types" {
  description = "Instance types for the GPU baseline node group."
  type        = list(string)
  default     = ["g7e.48xlarge"]
}

variable "gpu_baseline_min_size" {
  description = "Minimum (and fixed) number of GPU baseline nodes."
  type        = number
  default     = 2
}

variable "gpu_baseline_max_size" {
  description = "Maximum number of GPU baseline nodes."
  type        = number
  default     = 2
}

variable "gpu_baseline_desired_size" {
  description = "Desired number of GPU baseline nodes."
  type        = number
  default     = 2
}
...
variable "gpu_burst_spot_instance_types" {
  description = "Instance types for Karpenter Spot burst nodes. Broader selection increases Spot capacity availability."
  type        = list(string)
  default     = [
    "g7e.48xlarge", "g7e.24xlarge", "g7e.12xlarge",
    "g6e.48xlarge", "g6e.24xlarge", "g6e.12xlarge",
  ]
}

variable "gpu_burst_od_instance_types" {
  description = "Instance types for Karpenter On-Demand fallback burst nodes."
  type        = list(string)
  default     = ["g7e.48xlarge"]
}

variable "gpu_burst_spot_max_gpus" {
  description = "Maximum number of GPUs across all Spot burst nodes."
  type        = number
  default     = 80
}

variable "gpu_burst_od_max_gpus" {
  description = "Maximum number of GPUs across all On-Demand fallback burst nodes."
  type        = number
  default     = 40
}
